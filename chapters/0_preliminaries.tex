...
\begin{chapter}{Introduction}
 ...
 \begin{section}{Kernel Methods}
  ...
  \begin{subsection}{Translation Invariant Kernels}
      ...
      shift-invariant and it doesn't converge, in expectation, to the original kernel function $k(x-y)$ for $M \to +\infty$; a common way to tackle this issue is to consider:
      $$
      ...
      \begin{align*}
          ...
          \\
                                         & = \frac{1}{M}\E_{\substack{\omega_j \sim \Lambda \\ \xi_j \sim \mathcal{U}}}\left(\sum_{j=1}^M \cos(\omega_j^T(x-y))+\sum_{j=1}^M \cos(\omega_j^T(x+y)+2\xi_j)\right) \\
                                         & = \E(\cos(\omega^T(x-y))) = k(x,y)
      \end{align*}
      where we average over $\xi \sim \mathcal{U}[0, 2\pi]$ and $\omega \sim \Lambda$.
      \vspace{5pt}
      \newline As previously stated, our focus will be reducing the amount of memory needed to store the feature map used in learning algorithms.
