...
\begin{chapter}{Introduction}
 ...
 \begin{section}{Kernel Methods}
  ...
  \begin{subsection}{Translation Invariant Kernels}
      ...
      \begin{definition}
          A kernel map $k$ is called translation invariant if it satisfies the following property:
          $$k(x,y) = k^{\Delta }(x-y)$$
          for all $x, y \in \mathcal{X}$, where $\Delta$ is the Laplace-Beltrami operator on the input space $\mathcal{X}$ and $k^{\Delta}$ denotes the application of the operator to the function $k$.
      \end{definition}
      Translation invariant kernels are particularly interesting since the following theorem, due to Bochner, holds:

      \begin{theorem}
          A kernel map $k$ is called translation invariant if and only if it is a Fourier transform of a positive finite measure on $\R^d$, i.e.
          $$k(x, y) = k^\Delta(x-y) = \int_{\R^d} e^{i \omega^T (x-y)} \, d \Lambda(\omega)$$
          for some positive finite measure $\Lambda$ on $\R^d$.
      \end{theorem}

      Now, if we assume the kernel to be normalized, i.e. $k(0)=1$, then we can rewrite the condition above as:
      $$k^\Delta(u) = \E_{\omega \sim \Lambda}[e^{i \omega^T u}];$$
      hence exploiting the law of large numbers, we can approximate the kernel function with a finite sum:
      $$k^\Delta (x-y) \approx \frac{1}{M}\sum_{j=1}^M e^{i \omega_j^T (x-y)} = \innerprod{\frac{1}{\sqrt{M}}e^{i \Omega^T x}}{\frac{1}{\sqrt{M}}e^{i \Omega^T y}}_\C, \quad \Omega = (\omega_1, \dots, \omega_M) \in \R^{d\times M} $$
      hence we can define:
      \begin{definition}
          Let $k$ be a normalized translation invariant kernel. We define the Random Fourier feature map as:
          $$z_{RFF}(x) = \frac{1}{\sqrt{M}}e^{i \Omega^T x}, \quad x \in \R^d$$
          with $\Omega = (\omega_1, \dots, \omega_M) \in \R^{d\times M}$ where each $\omega_j$ is drawn from $\Lambda$, the probability distribution on $\R^d$ associated with the kernel $k$.
          The corresponding kernel function is:
          $$
              k_{RFF}(x,y) = \langle z_{RFF}(x), z_{RFF}(y) \rangle_{\C} = \frac{1}{M}e^{i \Omega^T (x-y)}
          $$
          which is shift invariant and converges, in expectation, to the original kernel function $k(x-y)$ for $M \to +\infty$.

      \end{definition}
      Since we weal deal only with real kernels, we consider only the real part of the Fourier function:
      $$\Re(e^{i \omega^T x}) = \cos(\omega^T x),$$
      but in general we have: $\innerprod{cos(\omega^t x)}{cos(\omega^t y)}_\R \neq cos(\omega^t (x-y))$, and so the corresponding approximated kernel is not
      shift-invariant and it doesn't converges, in expectation, to the original kernel function $k(x-y)$ for $M \to +\infty$; a common way to tackle this issue is to consider:
      $$
          z_{cos}(x) = \sqrt{\frac{2}{M}}cos(\omega^T x + \xi), \quad \xi \sim \mathcal{U}[0, 2\pi],
      $$
      in such a way:
      \begin{align*}
          \E(\innerprod{z_k(x)}{z_k(y)}) & = \frac{2}{M}\E (cos(\omega^T x + \xi)cos(\omega^T y + \xi ))
          \\
                                         & = \frac{2}{M}\E\left(\frac{1}{2}cos(\omega^T(x-y))+\frac{1}{2}cos(\omega^T(x+y)+2\xi)\right) \\
                                         & = \frac{1}{M}\E(cos(\omega^T(x-y))) = k(x,y)
      \end{align*}
      where we average over $\xi \sim \mathcal{U}[0, 2\pi]$ and $\omega \sim \Lambda$.
      \vspace{5pt}
      \newline As previously stated, our focus will be reducing the ammount of memory needed to store the feature map used in learning algorithms.
      To do so, instead of storing the full precision feature map $z(x) \in \R^M$, we can store a quantized version of it, $z_q(x) \in \R^M$, which uses 
      less numerical bits. We use the following procedure: 
      \begin{enumerate}
        \item given $M$ and $b$, we split the interval $[-\frac{1}{\sqrt{M}}, \frac{1}{\sqrt{M}}]$ in $2^b-1$ equal intervals. 
        \item for each coordinate $j \in \{1, \dots, M\}$, and each $x \in \R^d$, we quantize the coordinate $z_j(x)$ by taking the closest extreme point of the 
         belonging interval or by a stochastic quantization procedure depending on the value of $z_j(x)$ in the relative interval.  
      \end{enumerate}
