...
\begin{chapter}{Introduction}
 ...
 \begin{section}{Kernel Methods}
  ...
  \begin{subsection}{Translation Invariant Kernels}
      ...
      $$
          z_{cos}(x) = \sqrt{\frac{2}{M}}cos(\omega^T x + \xi), \quad \xi \sim \text{Unif}[0, 2\pi],
      $$
      in such a way:
      \begin{align*}
           & \E(\innerprod{z_k(x)}{z_k(y)})  = \frac{2}{M}\E (cos(\omega^T x + \xi)cos(\omega^T y + \xi ))
          \\ & = \frac{2}{M}\E(cos(\omega^T x)cos(\omega^T y)-cos(\omega^Tx)cos(\xi)-cos(\omega^T y)cos(\xi)+cos^2(\xi)) \\
           & =
      \end{align*}
      where we average over $\xi$ uniformely distributed in $[0, 2\pi]$ and $\Omega \sim \Lambda^m$ , 
